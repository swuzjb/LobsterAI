...
const CoworkView: React.FC<CoworkViewProps> = ({
  ...
  const handleStartSession = async (
    ...
    try {
      ...
      // Resolve effective working directory: agent-specific override takes priority over global config
      const effectiveCwd =
        currentAgent?.id !== 'main' && currentAgent?.workingDirectory?.trim()
            ...
            <div className="shadow-glow-accent rounded-2xl">
              <CoworkPromptInput
                ...
                onWorkingDirectoryChange={async (dir: string) => {
                  if (currentAgent && currentAgent.id !== 'main') {
                    await agentService.updateAgent(currentAgent.id, { workingDirectory: dir });
                  } else {
