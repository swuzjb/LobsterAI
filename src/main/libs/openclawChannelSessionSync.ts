...
export interface ChannelSessionSyncDeps {
  ...
  /** Optional: resolve a per-agent working directory override by agentId. Returns empty string if none. */
...
export class OpenClawChannelSessionSync {
  ...
  private readonly getAgentWorkingDirectory: (agentId: string) => string;
  ...
  constructor(deps: ChannelSessionSyncDeps) {
    ...
    this.getAgentWorkingDirectory = deps.getAgentWorkingDirectory ?? (() => '');
    this.resolveJobName = deps.resolveJobName ?? null;
  }

  /**
   * Resolve the working directory for a session belonging to the given agentId.
   * Uses the agent's custom workingDirectory if set; falls back to the global default cwd.
   */
  private resolveCwdForAgent(agentId: string): string {
    if (agentId && agentId !== 'main') {
      const custom = this.getAgentWorkingDirectory(agentId);
  ...
  resolveOrCreateSession(sessionKey: string): string | null {
    ...
    if (existingMapping) {
      ...
      if (session) {
        ...
        if (existingMapping.agentId !== currentAgentId) {
          ...
          const cwd = this.resolveCwdForAgent(currentAgentId);
    ...
    const cwd = this.resolveCwdForAgent(agentId);
    console.log(
